    ...
    def buildTree(self, expr):
        ...
        while i < len(expr):
            ...
            if(char in self.nums):
                ...
                elif(self.ops.get(char) < self.ops.get(current.content) or char == "^"):
                    new_node = Node4(char)
                    if(current.r_child):    
                        #print(char,"goes BOTTOM RIGHT")
        ...
        while(i < len(string)):
            if(string[i] == '('):
                ...
                if(in_par == 0):
                    ...
                    new_string = new_string + "[" + str(self.calculate(string[start+1:end])) + "]" 
            i += 1
...
if __name__ == "__main__":
    ...
    #test = Calculator4()
    #print(test.calculate("n(8*10^9-3.19*10^7)-o(8*a+b^b)-q(q(3))/i(6.666)-a+b*p(18^2-3^11)*p(18^2-3^11)+m(11174148)+n(8*10^9-3.19*10^7)+l(878791)"))    
